...
public class Vacuum {
    ...
    Float batteryNeededToGetBackToBase = 0F;
...
*/
    ...
    public Float getBatteryRequiredForFloorType(ThingsInRoom floorType) {
        if (floorType.equals(ThingsInRoom.BAREFLOOR)) {
            ...
            return 2F;
        } else if (floorType.equals(ThingsInRoom.HIGHPILE)) {
            return 3F;
        } else {
            return 0F;
        }
    ...
    private void consumeBatteryForMovement(Point currentLocation, Point nextPoint, Boolean isGoBackToBase) {
        final ThingsInRoom currentLocationFloorType = room.checkTypeOfFloor(currentLocation);
        final ThingsInRoom nextPointFloorType = room.checkTypeOfFloor(nextPoint);
        Float powerRequired = 0F;
        powerRequired += getBatteryRequiredForFloorType(currentLocationFloorType);
        powerRequired += getBatteryRequiredForFloorType(nextPointFloorType);


        if (powerRequired != 0) {
            powerRequired /= 2;
        }
        batteryLife -= powerRequired;

        batteryNeededToGetBackToBase = isGoBackToBase ? batteryNeededToGetBackToBase - powerRequired : batteryNeededToGetBackToBase + powerRequired;

    }

    private void consumeBatteryForMovement(Point currentLocation, Point nextPoint) {
    ...
    public void goBackToBase() {
        if (!returnlist.isEmpty()) {
            ...
            System.out.println("PowerNeededToBase:" + batteryNeededToGetBackToBase);
            currentLocation = lastPoint;
            try {
                Thread.sleep(500);
            } catch (Exception ex) {
                System.out.print("Encountered exception" + ex);
            }
        }
    ...
    public Point shutDown(Point p) {
        System.out.println("THERE IS NO WHERE TO GO>   SHUTTING DOWN at location " + p.toString());
